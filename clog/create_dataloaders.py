...
def create_train_valid_data_loaders(load_train, labels_train, load_test, labels_test, pad_len, batch_size):
    train_data = TensorDataset(torch.tensor(get_padded_data(load_train, pad_len=pad_len), dtype=torch.long), torch.tensor(labels_train, dtype=torch.long))
    train_sampler = RandomSampler(train_data)
    train_dataloader = DataLoader(train_data, sampler=train_sampler, batch_size=batch_size)


    test_data = TensorDataset(torch.tensor(get_padded_data(load_test, pad_len=pad_len), dtype=torch.long), torch.tensor(labels_test, dtype=torch.long))
