...
def string_to_list(x: str):
    ...
    Args:
        x (str): string with list format like: '[1 2 3 5]'

    Returns:
        np.array: np array of objects which would be np.array([1, 2, 3, 5]) in the above example
    """
...
# 60s/60s_results.csv
data = pd.read_csv("data/clusters/300s_clusters_30_.csv")
data["number_error_msgs"] = data.anom_label.apply(lambda x: convert_string_float(x))
data["round_1"] = data.round_1.apply(string_to_list)
data["round_2"] = data.round_2.apply(string_to_list)
# data["round_"] =


data_round_1 = data["round_1"]
data_round_2 = data["round_2"]
...
def sequential_dataset(data_round_1, round_):
    ...
    for test_id in test_id_groups.keys():
        ...
        for time_int in test_id_time_seq.keys():
            if round_ == 1:
                ...
                tests_1[str(test_id) + "_test_id_" + time_int] = (
                    tmp.pred_kmeans.values,
                    ...
                    " ".join(
                        [x for x in tmp.assertion_error_round_1.values[0].replace("['", "").replace("']", "").rsplit()]
                    ),
                    " ".join(
                        [x for x in tmp.round_1_final_status.values[0].replace("['", "").replace("']", "").rsplit()]
                    ),
                ...
                tmp = z.loc[test_id_time_seq[time_int]].loc[
                    ...
                    [
                        ...
                        "clog_sequence_parent_service",
                        "app_error2",
                        "assertion_error_round_2",
                        "round_2_final_status",
                        "anom_label",
                        "Content",
                        "parent_service",
                        "encoded_labels",
                ...
                tests_1[str(test_id) + "_test_id_" + time_int] = (
                    ...
                    " ".join(
                        [
                            x
                            for x in tmp.clog_sequence_parent_service.values[0]
                            .replace("['", "")
                            .replace("']", "")
                            .rsplit()
                        ]
                    ...
                    " ".join(
                        [x for x in tmp.assertion_error_round_2.values[0].replace("['", "").replace("']", "").rsplit()]
                    ),
                    " ".join(
                        [x for x in tmp.round_2_final_status.values[0].replace("['", "").replace("']", "").rsplit()]
                    ),
                    " ".join(x for x in tmp.Content.values),
                    tmp.anom_label.values.sum(),
                    tmp.parent_service.values,
    ...
    data_round_1.columns = [
        "ID",
        "cluster_sequences",
        "sequence_KPI",
        "cluster_sequences_parent_service",
        "app_error1",
        "assertion_error_round_1",
        "round_1_final_status",
        "Content",
...
def create_sequence_of_clusters(data_round_1):
    ...
    for test_id in np.unique(data_round_1.test_id):
        ...
        d[test_id] = (
            ...
            seq,
            seq1,
            tmp.app_error1.iloc[0],
            tmp.assertion_error_round_1.iloc[0],
            tmp.round_1_final_status.iloc[0],
            " ".join(x for x in tmp.Content.values),
            tmp.sequence_KPI.sum(),
        )
...
data_round_1["clog_sequence_parent_service"] = (
    data_round_1.pred_kmeans.apply(lambda x: str(x)) + data_round_1.parent_service
)
data_round_2["clog_sequence_parent_service"] = (
    data_round_2.pred_kmeans.apply(lambda x: str(x)) + data_round_2.parent_service
)
...
cluster_seq.to_csv("data/clustered/sequential_data" + TIME_INTERVAL + "_clusters_" + str(n_clusters) + "_.csv")
cluster_seq_round1.to_csv(
    "data/clustered/sequential_data_round1_" + TIME_INTERVAL + "_clusters_" + str(n_clusters) + "_.csv"
)
cluster_seq_round2.to_csv(
    "data/clustered/sequential_data_round2_" + TIME_INTERVAL + "_clusters_" + str(n_clusters) + "_.csv"
)

CLog_data.to_csv("data/clustered/classification_CLog_" + TIME_INTERVAL + "_clusters_" + str(n_clusters) + "_.csv")
tf_idf_data.to_csv("data/clustered/classification_TFIDF_" + TIME_INTERVAL + "_.csv")
#
#
data_round_s = pd.concat([data_round_1, data_round_2], axis=0)
data_round_s.to_csv(
    "data/clustered/novel_seq_" + TIME_INTERVAL + "_clusters_" + str(n_clusters) + "_individual_time_window_.csv"
)
...
for idx in np.unique(data_round_1.test_id):
    dataset.append(
        (
            "".join(x for x in data_round_1[data_round_1.test_id == idx].encoded_labels.values),
            data_round_1[data_round_1.test_id == idx].sequence_KPI.sum(),
            str(int(idx)) + "_testID_1",
        )
    )
...
for idx in np.unique(data_round_2.test_id):
    dataset.append(
        (
            "".join(x for x in data_round_2[data_round_2.test_id == idx].encoded_labels.values),
            data_round_2[data_round_2.test_id == idx].sequence_KPI.sum(),
            str(int(idx)) + "_testID_2",
        )
    )
...
to_save.to_csv(
    "data/clustered/classification_TFIDF_" + TIME_INTERVAL + "_clusters_" + str(n_clusters) + "_.csv", index=False
)
...
# to_save["target"] = cluster_seq.final_status.values
# to_save["ID"] = one_dataset_.ID
# to_save.to_csv("/home/matilda/PycharmProjects/FailurePrediction/4_analysis/clog/data/NOVA/resources/"
# + TIME_INTERVAL
# +"/classification_data/classification_TFIDF_"+TIME_INTERVAL+"_OTHER_DATA_.csv", index=False)
